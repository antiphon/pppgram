...
x <- spatstat.random::rThomas(kappa = 100/10, mu = 10, 0.05, nsim = 150)
xv <- lapply(x, \(b) periodogram_iso(b, t = k, taper_a = 0) |> as.data.frame()) |> 
  do.call(what = rbind)
...
x2 <- spatstat.random::rMaternII(kappa = maternII_generator_intensity(100, 0.05), 
                                0.05, nsim = 150)
x2v <- lapply(x2, \(b) 
              periodogram_iso(b, t = k, taper_a = 0) |> as.data.frame()) |> 
  do.call(what = rbind)
...
bias <- pppgram:::pppgram_iso_bias(c(1,1), k, 5)
